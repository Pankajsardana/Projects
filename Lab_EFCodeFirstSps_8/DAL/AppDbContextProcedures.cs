...
namespace DAL
{
    ...
    public class AppDbContextProcedures
    {
        ...
        public Employee UspGetEmployee(int EmployeeId)
        {
            ...
            using (var command = _context.Database.GetDbConnection().CreateCommand())
            {
                command.CommandText = "usp_getemployee"; command.CommandType = CommandType.StoredProcedure; var parameter = new SqlParameter("EmployeeId", EmployeeId); command.Parameters.Add(parameter); _context.Database.OpenConnection(); using (var reader = command.ExecuteReader()) { while (reader.Read()) { employee.EmployeeId = reader.GetInt32("EmployeeId"); employee.Name = reader.GetString("Name"); employee.Address = reader.GetString("Address"); employee.DepartmentId = reader.GetInt32("DepartmentId"); break; } }
                _context.Database.CloseConnection();
            }
            return employee;
        }
